...
#formatting data
data<-read.csv('ovca420.csv',header = TRUE) 
...
controls<-read.csv('ovca420_controls.csv',header = TRUE) 
...
td_svm<-as.data.frame(td_new) #no noise
td_svm2 <- as.data.frame(td_new_noise) # noise

td_pred<-as.data.frame(td_validation)
td_pred_noise<-as.data.frame(td_validation_noise)
...
#ggplot(vr)
vr2<-vr$importance
write.csv(vr2,'ovca420_no_noise.csv')
