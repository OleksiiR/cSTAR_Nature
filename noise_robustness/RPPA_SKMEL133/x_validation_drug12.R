...
#formatting data
data<-read.csv('drug12.csv',header = TRUE) 
#data<-read.csv('drug12_red.csv',header = TRUE) 
rownames(data)<-data[,1]
data<-data[,-1]
testdata<-(data) #no transpose
td<-as.data.frame(testdata)

library(janitor)
td<-remove_constant(td)

td$label<-factor(td$label,labels=c("Yes","No"))
table(td$label)
...
#ggplot(vr)
vr2<-vr$importance
write.csv(vr2,'drug12_noise.csv')
